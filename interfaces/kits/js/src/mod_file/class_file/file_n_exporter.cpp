    ...
    while ((ptr = readdir(pDir)) != nullptr) {
        if (strcmp(ptr->d_name, ".") != 0 && strcmp(ptr->d_name, "..") != 0) {
            if (ptr->d_type == DT_DIR) {
                if (!Rmdirs(path + "/" + ptr->d_name)) {
                    return false;
                }
            } else {
                if (remove((path + "/" + ptr->d_name).c_str()) != 0) {
                    closedir(pDir);
                    return false;
                }
            }
...
string ConvertUri(napi_env env, string path)
{
    AppExecFwk::Ability* ability = AbilityHelper::GetJsAbility(env);
    string pathData = ability->GetDataDir();
    string pathHead = "";
    if (path.find(pathData) != path.npos) {
        pathHead = "internal://app";
        path.replace(0, pathData.length(), pathHead);
    } else {
        string pathCache = ability->GetCacheDir();
        if (path.find(pathCache) != path.npos) {
            pathHead = "internal://cache";
            path.replace(0, pathCache.length(), pathHead);
        } else {
            return "error !";
        }
    }
...
napi_value FileNExporter::Mkdir(napi_env env, napi_callback_info info)
{
    ...
    bool succ = false;
    AsyncMkdirCallbackInfo *asyncCallbackInfo = new AsyncMkdirCallbackInfo {
        .env = env,
        .asyncWork = nullptr,
    };
    ...
    napi_create_async_work(
        env,
        nullptr,
        NVal::CreateUTF8String(env, "ResourceName").val_,
        [](napi_env env, void *data) {
            AsyncMkdirCallbackInfo *asyncCallbackInfo = (AsyncMkdirCallbackInfo *)data;
        ...
        [](napi_env env, napi_status status, void *data) {
            ...
            if (asyncCallbackInfo->result == SUCCESS) {
                ...
                CallBackError(env, asyncCallbackInfo->callback[COMMON_NUM::ONE], 
                              "make directory failed", FILE_IO_ERROR);
            }
            CallComplete(env, asyncCallbackInfo->callback[COMMON_NUM::TWO]);
...
napi_value FileNExporter::Rmdir(napi_env env, napi_callback_info info)
{
    ...
    bool succ = false;
    AsyncRmdirCallbackInfo *asyncCallbackInfo = new AsyncRmdirCallbackInfo {
        .env = env,
        .asyncWork = nullptr,
    };
    tie(succ, asyncCallbackInfo->callback[COMMON_NUM::ZERO], asyncCallbackInfo->callback[COMMON_NUM::ONE], 
    ...
    napi_create_async_work(
        ...
        NVal::CreateUTF8String(env, "ResourceName").val_,
        [](napi_env env, void *data) {
            AsyncRmdirCallbackInfo *asyncCallbackInfo = (AsyncRmdirCallbackInfo *)data;
            string path = asyncCallbackInfo->url;
            asyncCallbackInfo->result = FAILD;
            asyncCallbackInfo->errorType = FILE_IO_ERROR;
        ...
        [](napi_env env, napi_status status, void *data) {
            ...
            if (asyncCallbackInfo->result == SUCCESS) {
                ...
                CallBackError(env, asyncCallbackInfo->callback[COMMON_NUM::ONE],
                              "remove directory failed", FILE_IO_ERROR);
...
napi_value FileNExporter::Get(napi_env env, napi_callback_info info)
{
    ...
    AsyncGetCallbackInfo *asyncCallbackInfo = new AsyncGetCallbackInfo {
        .env = env, 
        .asyncWork = nullptr, 
    };
    bool succ = false;
    ...
    napi_create_async_work(
        ...
        NVal::CreateUTF8String(env, "ResourceName").val_,
        [](napi_env env, void *data) {
            AsyncGetCallbackInfo *asyncCallbackInfo = (AsyncGetCallbackInfo *)data;
            string path = asyncCallbackInfo->url;
            asyncCallbackInfo->result = FAILD;
            asyncCallbackInfo->errorType = FILE_IO_ERROR;
            struct stat buf;
            ...
            if (statPath == COMMON_NUM::ZERO && stat((char*)path.c_str(), &buf) == COMMON_NUM::ZERO) {
                ...
                asyncCallbackInfo->lastMT = buf.st_mtime * 1000 + (int64_t)((buf.st_mtim).tv_nsec / 1000000);
        ...
        [](napi_env env, napi_status status, void *data) {
            ...
            if (asyncCallbackInfo->result == SUCCESS) {
                ...
                for (auto filename : asyncCallbackInfo->subFiles) {
                    napi_set_property(env, subFilesNapi, NVal::CreateInt32(env, i).val_, 
                                      NVal::CreateUTF8String(env, ConvertUri(env, filename)).val_);
                    i = i + 1;
                }
                ...
                objn.AddProp("uri", NVal::CreateUTF8String(env, ConvertUri(env, asyncCallbackInfo->url).c_str()).val_);
                objn.AddProp("type", NVal::CreateUTF8String(env, asyncCallbackInfo->type).val_);
                objn.AddProp("subFiles", subFilesNapi);

                CallBackSuccess(env, asyncCallbackInfo->callback[0], COMMON_NUM::ONE, objn.val_);
            } else if (asyncCallbackInfo->errorType == FILE_PATH_ERROR) {
                CallBackError(env, asyncCallbackInfo->callback[COMMON_NUM::ONE], "file not exist", FILE_PATH_ERROR);
            } else if (asyncCallbackInfo->errorType == FILE_IO_ERROR) {
...
napi_value FileNExporter::List(napi_env env, napi_callback_info info)
{
    ...
    AsyncListCallbackInfo *asyncCallbackInfo = new AsyncListCallbackInfo {
        .env = env,
        .asyncWork = nullptr,
    };
    bool succ = false;
    ...
    napi_create_async_work(
        ...
        NVal::CreateUTF8String(env, "ResourceName").val_,
        [](napi_env env, void *data) {
            AsyncListCallbackInfo *asyncCallbackInfo = (AsyncListCallbackInfo *)data;
            string path = asyncCallbackInfo->url;
            asyncCallbackInfo->result = FAILD;
                ...
                if (!getStat) {
                    ...
                    for (auto ph : fileNames) {
                        ...
                        if (r == 0) {
                            ...
                            } else if ((tmp.st_mode & S_IFMT) == S_IFREG) {
                                fi.type = TYPE_FILE;
                            }
                            fi.length = tmp.st_size;
                            fi.lastModifiedTime = tmp.st_mtime * 1000 + (int64_t)((tmp.st_mtim).tv_nsec / 1000000);
                            fi.uri = ph;
        ...
        [](napi_env env, napi_status status, void *data) {
            ...
            if (asyncCallbackInfo->result == SUCCESS) {
                ...
                for (auto fileInfo: asyncCallbackInfo->fileList) {
                    ...
                    objt.AddProp("uri", NVal::CreateUTF8String(env, ConvertUri(env, fileInfo.uri).c_str()).val_);
...
napi_value FileNExporter::Copy(napi_env env, napi_callback_info info)
{
    ...
    bool succ = false;
    AsyncCopyCallbackInfo *asyncCallbackInfo = new AsyncCopyCallbackInfo { 
        .env = env, 
        .asyncWork = nullptr, 
    };
    tie(succ, asyncCallbackInfo->callback[COMMON_NUM::ZERO], asyncCallbackInfo->callback[COMMON_NUM::ONE], 
    ...
    napi_create_async_work(
        ...
        NVal::CreateUTF8String(env, "ResourceName").val_,
        [](napi_env env, void *data) {
            AsyncCopyCallbackInfo *asyncCallbackInfo = (AsyncCopyCallbackInfo *)data;
            string path = asyncCallbackInfo->url;
            ...
            if (statPath == COMMON_NUM::ZERO && statDst == ENOENT) {
                ...
                if (sfd.GetFD() != FAILD && ofd.GetFD() != FAILD 
                    && fstat(sfd.GetFD(), &statbf) != FAILD
...
napi_value FileNExporter::Move(napi_env env, napi_callback_info info)
{
    ...
    napi_create_async_work(
        ...
        NVal::CreateUTF8String(env, "ResourceName").val_,
        [](napi_env env, void *data) {
            AsyncMoveCallbackInfo *asyncCallbackInfo = (AsyncMoveCallbackInfo *)data;
            string path = asyncCallbackInfo->url;
            string pathDst = asyncCallbackInfo->urlDst;
            asyncCallbackInfo->result = FAILD;
            asyncCallbackInfo->errorType = FILE_IO_ERROR;
...
napi_value FileNExporter::Delete(napi_env env, napi_callback_info info)
{
    ...
    bool succ = false;
    AsyncDeleteCallbackInfo *asyncCallbackInfo = new AsyncDeleteCallbackInfo {
        .env = env,
        .asyncWork = nullptr,
    };
    tie(succ, asyncCallbackInfo->callback[COMMON_NUM::ZERO], asyncCallbackInfo->callback[COMMON_NUM::ONE], 
    ...
    napi_create_async_work(
        ...
        NVal::CreateUTF8String(env, "ResourceName").val_,
        [](napi_env env, void *data) {
            AsyncDeleteCallbackInfo *asyncCallbackInfo = (AsyncDeleteCallbackInfo *)data;
            string path = asyncCallbackInfo->url;
            asyncCallbackInfo->result = FAILD;
            int statPath = GetRealPath(path);
            if (statPath == ENOENT) {
...
napi_value FileNExporter::Access(napi_env env, napi_callback_info info)
{
    ...
    napi_create_async_work(
        ...
        NVal::CreateUTF8String(env, "ResourceName").val_,
        [](napi_env env, void *data) {
            AsyncAccessCallbackInfo *asyncCallbackInfo = (AsyncAccessCallbackInfo *)data;
            string path = asyncCallbackInfo->url;
            asyncCallbackInfo->result = FAILD;
            int statPath = GetRealPath(path);
            if (statPath == ENOENT) {
...
napi_value FileNExporter::WriteText(napi_env env, napi_callback_info info)
{
    ...
    AsyncWriteCallbackInfo *asyncCallbackInfo = new AsyncWriteCallbackInfo {
        .env = env,
        .asyncWork = nullptr
    };
    bool succ = false;
    ...
    napi_create_async_work(
        ...
        NVal::CreateUTF8String(env, "ResourceName").val_,
        [](napi_env env, void *data) {
            AsyncWriteCallbackInfo *asyncCallbackInfo = (AsyncWriteCallbackInfo *)data;
            string path = asyncCallbackInfo->url;
...
napi_value FileNExporter::WriteArrayBuffer(napi_env env, napi_callback_info info)
{
    ...
    bool succ = false;
    AsyncWriteBufferCallbackInfo *asyncCallbackInfo = new AsyncWriteBufferCallbackInfo { 
        .env = env, 
        .asyncWork = nullptr, 
    };
    tie(succ, asyncCallbackInfo->callback[COMMON_NUM::ZERO], asyncCallbackInfo->callback[COMMON_NUM::ONE], 
    ...
    napi_create_async_work(
        ...
        NVal::CreateUTF8String(env, "ResourceName").val_,
        [](napi_env env, void *data) {
            AsyncWriteBufferCallbackInfo *asyncCallbackInfo = (AsyncWriteBufferCallbackInfo *)data;
            string path = asyncCallbackInfo->url;
            asyncCallbackInfo->result = FAILD;
            asyncCallbackInfo->errorType = FILE_IO_ERROR;
            int fd = -1;
...
napi_value FileNExporter::ReadText(napi_env env, napi_callback_info info)
{
    ...
    bool succ = false;
    AsyncReadCallbackInfo *asyncCallbackInfo = new AsyncReadCallbackInfo {
        .env = env,
        .asyncWork = nullptr,
    };
    tie(succ, asyncCallbackInfo->callback[COMMON_NUM::ZERO], asyncCallbackInfo->callback[COMMON_NUM::ONE], 
    ...
    napi_create_async_work(
        ...
        NVal::CreateUTF8String(env, "ResourceName").val_,
        [](napi_env env, void *data) {
            AsyncReadCallbackInfo *asyncCallbackInfo = (AsyncReadCallbackInfo *)data;
            string path = asyncCallbackInfo->url;
            asyncCallbackInfo->result = FAILD;
            asyncCallbackInfo->errorType = FILE_IO_ERROR;
            int statPath = GetRealPath(path);
            if (statPath == COMMON_NUM::ZERO) {
                ...
                if (fd != FAILD) {
                    if (result != FAILD) {
                        ...
                        if (read(fd, buffer, buf.st_size) != FAILD) {
                            asyncCallbackInfo->result = SUCCESS;
...
napi_value FileNExporter::ReadArrayBuffer(napi_env env, napi_callback_info info)
{
    ...
    AsyncReadBufferCallbackInfo *asyncCallbackInfo = new AsyncReadBufferCallbackInfo {
        .env = env,
        .asyncWork = nullptr,
    };
    bool succ = false;
    ...
    napi_create_async_work(
        ...
        NVal::CreateUTF8String(env, "ResourceName").val_,
        [](napi_env env, void *data) {
            AsyncReadBufferCallbackInfo *asyncCallbackInfo = (AsyncReadBufferCallbackInfo *)data;
            string path = asyncCallbackInfo->url;
            asyncCallbackInfo->result = FAILD;
            asyncCallbackInfo->errorType = FILE_IO_ERROR;
            int statPath = GetRealPath(path);
            if (statPath == COMMON_NUM::ZERO) {
                ...
                if (fd != FAILD) {
                    if (result != FAILD) {
                        int32_t begin = 
                            buf.st_size < asyncCallbackInfo->position ? buf.st_size : asyncCallbackInfo->position;
                        int32_t len = asyncCallbackInfo->length == 0 ? buf.st_size - begin : asyncCallbackInfo->length;
                        char *buffer = new char[len + 1];
                        lseek(fd, begin, SEEK_CUR);
                        if (read(fd, buffer, len) != FAILD) {
                            asyncCallbackInfo->result = SUCCESS;

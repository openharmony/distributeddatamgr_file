...
#ifndef FOUNDATION_STORAGE_INCLUDE_PARCEL_MACRO_H
#define FOUNDATION_STORAGE_INCLUDE_PARCEL_MACRO_H
...
#define READ_PARCEL_AND_RETURN_FALSE_IF_FAIL(type, parcel, data)                                             \
    do {                                                                                                     \
        if (!(parcel).Read##type(data)) {                                                                    \
            SSLOG_E("fail to read %{public}s type from parcel", #type);                                      \
            return false;                                                                                    \
        }                                                                                                    \
    } while (0)
...
#define WRITE_PARCEL_AND_RETURN_FALSE_IF_FAIL(type, parcel, data)                                            \
    do {                                                                                                     \
        if (!(parcel).Write##type(data)) {                                                                   \
            SSLOG_E("fail to write %{public}s type into parcel", #type);                                     \
            return false;                                                                                    \
        }                                                                                                    \
    } while (0)
} // namespace DS

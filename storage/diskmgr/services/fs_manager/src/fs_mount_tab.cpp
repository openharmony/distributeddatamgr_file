...
constexpr int MIN_MOUNT_OPTION = 3;
constexpr int MAX_MOUNT_OPTION = 5;
constexpr int WAIT_MAX_COUNT = 10;
constexpr int MAX_WAIT_TIME = 500000;
...
void WaitForFile(const std::string &source, unsigned int maxCount)
{
    ...
    do {
        usleep(waitTime);
        count++;
    } while ((stat(source.c_str(), &sourceInfo) < 0) && (errno == ENOENT) && (count < maxCount));
    if (count == maxCount) {
        SSLOG_E("wait for file:%s failed after retry %d. times", source.c_str(), maxCount);
    }   
}
...
bool ParseFsMgrFlags(const std::string &strFsmgr, FsMountEntry &entry) {
    ...
    for (auto &item : flags) {
        ...
        if ("checkpoint=block" == item) {
            entry.fsMgrFlags.checkpointBlk = true;
            continue;
        } else if ("checkpoint=fs" == item) {

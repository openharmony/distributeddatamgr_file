...
tuple<bool, void *, size_t> NVal::ToTypedArray() const
{
    ...
    napi_value inArrayBuffer = nullptr;
    size_t byteOffset;
    size_t length;
    void *data = nullptr;
    napi_status status =
        napi_get_typedarray_info(env_, val_, &type, &length, (void **)&data, &inArrayBuffer, &byteOffset);
...
NVal NVal::CreateUint8Array(napi_env env, void *buf, size_t bufLen)
{
    ...
    napi_create_typedarray(env, napi_uint8_array, bufLen, outputBuffer, 0, &outputArray);
